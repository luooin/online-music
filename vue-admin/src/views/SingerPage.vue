  ...
  <div class="container">
    ...
    <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="性别" prop="sex" width="60" align="center">
        <template v-slot="scope">
          <div>{{ changeSingerSex(scope.row.sex) }}</div>
        </template>
      </el-table-column>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { changeSingerSex, routerManager, beforeImgUpload } = mixin();
    ...
    function deleteAll() {
      ...
      // 弹出确认框，确认后批量删除
      (proxy as any).$confirm("确定要删除选中的歌手吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
          .then(() => {
            Promise.all(idsToDelete.map(id => HttpManager.deleteSinger(id)))
                .then(() => {
