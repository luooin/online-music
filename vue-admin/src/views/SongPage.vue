...
export default defineComponent({
  ...
  setup() {
    ...
    function addSong() {
      ...
      const file = addSongForm.get("file") as File;
      if (file) {
        const fileName = file.name;
        const fileExtension = fileName.split('.').pop();
        if (fileExtension?.toLowerCase() !== "mp3") {
          (proxy as any).$message.error("请上传MP3格式的文件");
          return;
        }
      } else {
        (proxy as any).$message.error("请上传文件");
        return;
      }
    ...
    function deleteAll() {
      ...
      (proxy as any).$confirm("确定要删除选中的歌曲吗？", "提示", {
        ...
        type: "warning",
          ...
          .then(() => {
            Promise.all(idsToDelete.map(id => HttpManager.deleteSong(id)))
                .then(() => {
