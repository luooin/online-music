<template>
  <div class="yin-header">
    ...
    <yin-header-nav class="yin-header-nav" :styleList="headerNavList" :activeName="activeNavName"
...
export default defineComponent({
  ...
  setup() {
    ...
    function goPage(path, name) {    // 【/ChatRoom,聊天室】
      if (!path && !name) {     // 没有路径 名称 就返回首页
        changeIndex(NavName.Home);
        routerManager(RouterName.Home, {path: RouterName.Home});
      }else if(path=='/ChatRoom'&&proxy.$store.getters.token==false) {
        (proxy as any).$message({             // 未登录不能进进入聊天室
          message: "请先登录",
          type: "warning",
        });
      }else {
        changeIndex(name); // 导航栏名称}
