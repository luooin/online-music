...
async function getComment(id) {
  try {
    ...
    for (let index = 0; index < commentList.value.length; index++) {
      ...
      console.log("投投投"+resultUser.data[0].avator)
...
async function setSupport(id, up, userId) {
  ...
  const hasSupported = await HttpManager.testAlreadySupport({ commentId, userId });

  if (hasSupported.data) {
    up = up - 1;
    operatorR = await HttpManager.deleteUserSupport({ commentId, userId });
    result = await HttpManager.setSupport({ id, up });
    ...
    up = up + 1;
    operatorR = await HttpManager.insertUserSupport({ commentId, userId });
    result = await HttpManager.setSupport({ id, up });
    if (result.success && operatorR.success) {
      await getComment(playId.value);
      (proxy as any).$message({
